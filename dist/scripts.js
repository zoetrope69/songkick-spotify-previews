...
function sanitizeArtists(artists) {
  return artists.map(sanitizeString);
}
...
function sanitizeString(string) {
  return removeDiacritics(string)
    ...
    .replace(/(US)/g, '') // remove us modifier
    .replace(/(UK)/g, '') // remove uk modifier
    .replace(/(dj set)/g, '') // remove dj set info from name
    .replace(/dj/g, '') // remove dj from name
    .replace(/(official)/g, '') // remove official info from name
  ...
  return new Promise((resolve, reject) => {
    ...
    return fetch(url, { headers })
      ...
      .then(data => {
        ...
        let item = items.find(item => sanitizeString(item.name.toLowerCase()) === query);
  ...
  const html = `
    <li>
      <a href="${track.external_urls.spotify}" target="_blank" class="track-details">
        ...
        <ul>
          <li>${track.name}</li>
          <li>${track.artists.map(artist => artist.name).join(', ')}</li>
